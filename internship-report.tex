...
\usepackage[utf8]{inputenc} \usepackage[hidelinks]{hyperref}
...
\end{itemize}
\section{TLS Certificate Common Name Alteration Script}\label{sec:common-name-alteration}
...
Similarly to the script described in \autoref{sec:common-name-alteration}, this script is used to alter both the Legit MQTT Server's TLS Certificate and the Intermediate CA's TLS Certificate, the script is designed to have 3 inputs:
\begin{itemize}
	\item Path of the certificate to be altered.
	\item Path of the destination where the altered certificate will be saved.
	\item Path of the reference certificate that will be used to copy and paste the Common Name from.
...
Thanks to our literary research, we could identify the key concepts and key vulnerabilities on which we had to focus our efforts.

We could then produce a formally defined Test Suite, with Test Cases designed to expose possible weaknesses of a faulty TLS implementation in any Application, all the more so of a faulty TLS implementation in an MQTT Broker. This Test Suite was designed using simple concepts and intuitions, transforming them into a structured set of diagrams and tables.

Another important factor in this internship has been the setup of our Test Laboratory Environment, through a predominant use of the Docker framework features, which allowed us to simulate an MQTT over TLS connection between multiple actors. More specifically, it allowed us to create an actor representing the MQTT Client, and to get a local copy of the Tested Libraries' public Docker Container distributions, which would act as our MQTT Broker.

For some Libraries, we had to create our own Docker Container distribution based on a snapshot of the Library itself, due to a lack of a public distribution.

Through the setup of our Test Laboratory Environment, we managed to adapt those formal Test Cases in a reproducible manner, allowing us to recreate the same conditions and reach the same results over time. More specifically, our results showed that most of the tested Libraries are secure from the Transport Layer Security perspective, and we even managed to find a security threat on RouterOS version 7, which was not present on RouterOS version 6, in other words, we could even find a regression bug.
